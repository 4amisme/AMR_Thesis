...
try:
    ...
    # 3. Data Cleaning & Standardization (แก้ไขหัวตารางเป็นตัวเล็กเพื่อป้องกัน Error)
    df.columns = df.columns.str.strip().str.lower()
    
    # กำหนดชื่อคอลัมน์เป้าหมายหลังจากเปลี่ยนเป็นตัวเล็กแล้ว
    target_pattern_col = 'resistant_drug_classes' 

    # ตัดแถวที่ข้อมูลสำคัญเป็นค่าว่าง (ใช้ชื่อคอลัมน์ตัวเล็ก)
    ...
    for organism in all_organisms:
        ...
        # หา Top 5 Patterns (ใช้ชื่อคอลัมน์ตัวเล็ก)
        top_5_patterns = df_org[target_pattern_col].value_counts().nlargest(5).index.tolist()
        ...
        df_top5 = df_org[df_org[target_pattern_col].isin(top_5_patterns)]
        monthly_ward_counts = df_top5.groupby(['year', 'month', 'ward_type', target_pattern_col]).size().reset_index(name='pattern_count')
        ...
        for ward in final_df_org['ward_type'].unique():
            ...
            column_order = [
                ...
                target_pattern_col, 'pattern_count', 
