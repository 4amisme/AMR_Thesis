...
# 1. กำหนดเส้นทางไฟล์
input_path = '/Users/chanokchonkarinrak/Documents/GitHub/AMR_Thesis/data/MDR/AllYears_MDR_Pattern.csv'
mapping_path = '/Users/chanokchonkarinrak/Documents/GitHub/AMR_Thesis/MDR/spatiotemperal/province-hos-code.csv'
output_dir = '/Users/chanokchonkarinrak/Documents/GitHub/AMR_Thesis/MDR/spatiotemperal/aba_province'
...
try:
    ...
    # โหลดไฟล์ mapping สำหรับจังหวัด
    df_map = pd.read_csv(mapping_path)
    
    # สร้าง Dictionary สำหรับแมป laboratory -> province
    prov_mapping = df_map.drop_duplicates(subset=['laboratory']).set_index('laboratory')['province'].to_dict()
    
    # อัปเดตคอลัมน์ province ใน df หลัก โดยดึงจาก laboratory
    df['province'] = df['laboratory'].map(prov_mapping)
    ...
    for organism in all_organisms:
        ...
        # คำนวณจำนวนเคสรวมรายเดือน แยกตาม "จังหวัด" (ตัวหาร)
        monthly_total = df_org.groupby(['year', 'month', 'province']).size().reset_index(name='total_rows_in_month')

        # คำนวณจำนวนการเกิดของแต่ละ Pattern ใน Top 5 แยกตาม "จังหวัด"
        df_top5 = df_org[df_org['Resistant_Drug_Classes'].isin(top_5_patterns)]
        monthly_counts = df_top5.groupby(['year', 'month', 'province', 'Resistant_Drug_Classes']).size().reset_index(name='pattern_count')

        # รวมข้อมูลและคำนวณ %
        final_df = pd.merge(monthly_counts, monthly_total, on=['year', 'month', 'province'])
        ...
        # --- จัดลำดับคอลัมน์ใหม่ตามที่ต้องการ ---
        column_order = [
            'organism_full', 
            'year', 
            'month', 
        ...
        # เรียงลำดับตาม ปี เดือน จังหวัด และเปอร์เซ็นต์
        final_df = final_df.sort_values(['year', 'month', 'province', 'percentage'], ascending=[True, True, True, False])
